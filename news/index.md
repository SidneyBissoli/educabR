...
- [`get_ideb()`](https://sidneybissoli.github.io/educabR/reference/get_ideb.md)
  now supports the IDEB 2025 edition (released 2026-08-05). INEP changed
  the packaging for 2025: spreadsheets now come inside a `.zip` archive,
  which the pipeline downloads and extracts transparently. The cached
...
- [`get_ideb()`](https://sidneybissoli.github.io/educabR/reference/get_ideb.md)
  now raises a clear error when the `year` filter matches no IDEB
  edition (listing the editions actually present in the file), instead
...
- Dynamic year discovery no longer removes known years from
  ...
  when a verification request fails transiently — a network hiccup could
  make a valid year (e.g. 2022 for the Censo Escolar) unavailable for
  the whole session
  ([\#17](https://github.com/SidneyBissoli/educabR/issues/17)).
  Discovery now only *adds* newly published years to the known list.
...
- [`validate_year()`](https://sidneybissoli.github.io/educabR/reference/validate_year.md)
  ...
  for multi-year composition (issue
  [\#2](https://github.com/SidneyBissoli/educabR/issues/2)). Previously,
  passing `c(2017, 2019)` to any of the 13 affected getters (`get_cpc`,
  `get_idd`, `get_igc`, `get_capes`, `get_saeb`, `get_enem`,
  `get_enem_itens`, `get_enade`, `get_encceja`,
  `get_fundeb_distribution`, `get_fundeb_enrollment`,
  `get_censo_escolar`, `get_censo_superior`) hit either a cryptic
  `length > 1` error (R ≥ 4.2) or silently used only the first element
  (R \< 4.2).
  [`get_ideb()`](https://sidneybissoli.github.io/educabR/reference/get_ideb.md)
...
- [`get_fundeb_distribution()`](https://sidneybissoli.github.io/educabR/reference/get_fundeb_distribution.md):
  Download FUNDEB resource distribution data (years 2007-2026). Reads
  all sheets from STN Excel files and returns tidy long-format data with
...
- Data sources: Tesouro Transparente
  (`https://www.tesourotransparente.gov.br`) and FNDE
...
- [`get_capes()`](https://sidneybissoli.github.io/educabR/reference/get_capes.md):
  Download CAPES graduate education data (years 2013-2024).
- Supports 5 data types: programs (`"programas"`), students
  (`"discentes"`), faculty (`"docentes"`), courses (`"cursos"`), and
  theses/dissertations catalog (`"catalogo"`).
- Uses CKAN API to dynamically discover download URLs (CAPES URLs
  contain UUIDs).
- Data source: CAPES Open Data Portal
...
- [`get_cpc()`](https://sidneybissoli.github.io/educabR/reference/get_cpc.md):
  Download CPC data (years 2007-2019, 2021-2023; no 2020 edition).
...
- [`get_igc()`](https://sidneybissoli.github.io/educabR/reference/get_igc.md):
  Download IGC data (years 2007-2019, 2021-2023; no 2020 edition).
- Institutional quality indicator based on weighted CPC averages and
  CAPES scores.
- Files are in Excel format (xls/xlsx), except 2007 which is a 7z
  archive.
- Hardcoded URL map due to completely inconsistent INEP naming patterns.
...
- [`read_excel_safe()`](https://sidneybissoli.github.io/educabR/reference/read_excel_safe.md):
  Internal helper to read Excel files with error handling.
